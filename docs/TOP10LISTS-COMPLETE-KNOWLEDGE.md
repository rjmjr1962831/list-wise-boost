# DEPRECATED — Use PROJECT-KNOWLEDGE.md

**Superseded by:** [docs/PROJECT-KNOWLEDGE.md](./PROJECT-KNOWLEDGE.md)

---

# Top10Lists.us - Complete Project Knowledge (Legacy)
...
**NEVER store secrets in this document or commit them to GitHub.** Use `.env` (gitignored) or the respective service dashboards.

### AI Services
| Service | Use | Where |
|---------|-----|------|
| **Anthropic** | Prime/Luxury content | Supabase secrets / .env |
| **DeepSeek** | Main tier (90% cheaper) | Supabase secrets / .env |
| **OpenAI** | Fallback | Supabase secrets / .env |
...
| Service | Where |
|---------|-------|
| **Exa.ai** | Supabase secrets / .env |
| **GitHub Token** | .env as GITHUB_TOKEN |
| **Vercel API** | Vercel dashboard / .env |
| **ProxyScrape** | Supabase secrets / .env |
...
- **Token:** Use `GITHUB_TOKEN` from .env (never commit)
...
2. **Read** existing `docs/TOP10LISTS-COMPLETE-KNOWLEDGE.md`
3. **Integrate** new information into appropriate sections
4. **Check** for conflicts or superseded information (e.g., PrivateEmail → Google Workspace)
...
```bat
curl -s -H "Authorization: token %GITHUB_TOKEN%" -H "Accept: application/vnd.github.v3.raw" "https://api.github.com/repos/rjmjr1962831/list-wise-boost/contents/path/to/file.ts" -o file.ts
```
