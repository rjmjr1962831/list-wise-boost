...
- **Search query we ran:** Where we used a search query to gather data (e.g. press search, community), **include that search query in the payload** so recipients can recreate the search and verify. Store the query (or queries) when we run enrichment so the artifact can expose it (e.g. "Verification: To recreate our research, run: [query]").
...
| Community | `{{detailed_list}}` (Source: ProPublica 990s, editorial, org rosters) | **Per role: if ProPublica, use [ProPublica IRS Form 990, EIN {ein}]({filing_url})** |

**Community — sources we have in the DB**
