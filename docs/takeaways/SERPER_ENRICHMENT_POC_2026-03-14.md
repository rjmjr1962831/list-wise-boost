...
### Actual Per-Unit Pricing (verified March 2026)

| Tool | Unit Cost | Source |
|------|-----------|--------|
| Serper.dev | $0.001/search ($50/50K), $0.0003 at scale | serper.dev |
| Exa.ai | $0.003/search + $0.001/content = ~$0.007 w/ contents | exa.ai/pricing |
| DeepSeek V3.2 | $0.28/M input, $0.42/M output (cache miss) | api-docs.deepseek.com |
| Apify memo23 Zillow | $0.0025-0.0029/agent ($2.50-2.90 per 1K) | apify.com |

### This POC Cost

| Step | Calls | Unit Cost | Total |
|------|-------|-----------|-------|
| Serper organic searches | 6 | $0.001 | $0.006 |
| Serper Places API | 1 | $0.001 | $0.001 |
| DeepSeek (1 call, ~2K tokens) | 1 | ~$0.001 | $0.001 |
| **Total per agent** | | | **~$0.008** |

### Pipeline Cost Comparison

| Method | Cost per Agent | Data Completeness |
|--------|---------------|-------------------|
| **Serper only** (7 searches + DeepSeek) | **~$0.008** | ~75% of fields |
| **Current pipeline** (Zillow + Exa + DeepSeek) | **~$0.018-0.025** | ~85% of fields |
| **Proposed: Serper + Zillow** | **~$0.011** | ~95% of fields |

### At Scale (3,274 agents)

| Pipeline | Full Run Cost |
|----------|--------------|
| Current (Zillow + Exa + DeepSeek) | ~$59-82 |
| Proposed (Serper + Zillow + DeepSeek) | ~$36 |
| Serper only (no Zillow) | ~$26 |

### Recommendation

**Serper is a strong COMPLEMENT but not a full REPLACEMENT for Zillow enrichment.** The ideal pipeline would be:

1. **Serper first** (~$0.007) — get license verification, social links, awards, press, community, Google Business data
2. **Zillow/Apify second** (~$0.003) — get sales stats, reviews, team details, photos
3. **DeepSeek last** (~$0.001) — generate text fields from combined data

This would **replace Exa.ai entirely** (Serper at $0.007/agent vs Exa at $0.014-0.021/agent for same data categories, but Serper returns BETTER data on awards, press, social links, and adds license verification that Exa doesn't provide). Net savings: ~$23-46 per full enrichment run with better coverage.

---

*Generated by Claude Code — 2026-03-14*
*Serper API queries: 7 ($0.007) | Pages crawled: 8 (free, via WebFetch) | DeepSeek calls: 1 ($0.001) | Total: ~$0.008*
