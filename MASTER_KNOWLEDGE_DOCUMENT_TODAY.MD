...
**Location & access:** Lives at repo root (`MASTER_KNOWLEDGE_DOCUMENT_TODAY.MD`) for AI access (Cursor, Claude, Gemini). Must **never** be served on the website or deployed to any public URL.
...
- **Manually merge staging to main** — internal documents (this file, PENDING_UPDATES.md, docs/cursor-daily-updates.md, etc.) must NEVER reach main. **ALWAYS** run `npm run merge-to-main`, which excludes them. Manual `git merge` + `git push` pushes internal docs to production; forbidden.
- **Telling Robert to push internal docs to main** — When you update an internal document, do NOT say it needs to be pushed to main. That creates a loop every time. Say nothing about pushing to main.
...
### Home Page (Index.tsx)
* **Do not change layout or spacing** unless Robert explicitly requests it. Rule in `.cursor/rules/home-page.mdc`; editors have repeatedly broken the homepage. Safe to change: copy, meta tags, challenge question, links. Do not change section `py-*` or structure.
...
- **ryt** = Update MASTER_KNOWLEDGE_DOCUMENT_TODAY.MD with session knowledge (this file only; commit and push to staging)
- **takeaways** = Generate daily outcomes; template in `npm run takeaways`; master doc name in template is MASTER_KNOWLEDGE_DOCUMENT_TODAY.MD. Write to private repo or docs/cursor-daily-updates.md per project-knowledge.mdc.
...
1. **Sync State**: Read `MASTER_KNOWLEDGE_DOCUMENT_TODAY.MD` before any code changes.
...
*Version 5.7 - February 22, 2026*
*Changes from v5.6: Home page rule (do not change Index.tsx layout/spacing). Takeaways template references MASTER_KNOWLEDGE_DOCUMENT_TODAY.MD. Internal-doc update rule: do not tell Robert to push to main when updating internal docs (loop prevention).*
