...
**API Keys:**
- **Anon/Publishable:** `[STORED IN CLAUDE PROJECT]`
- **Service Role:** `[STORED IN CLAUDE PROJECT]`
...
**Auth Header:** `X-Enrichment-Key: [STORED IN CLAUDE PROJECT]`
...
| Service | Key | Use |
|---------|-----|-----|
| **Anthropic** | `[STORED IN CLAUDE PROJECT]` | Prime/Luxury content |
...
| Service | Key |
|---------|-----|
| **Exa.ai** | `[STORED IN CLAUDE PROJECT]` |
| **GitHub Token** | `[STORED IN CLAUDE PROJECT]` |
| **Vercel API** | `[STORED IN CLAUDE PROJECT]` (named "Claude Token") |
| **ProxyScrape** | Host: `rp.scrapegw.com:6060` Auth: `[STORED IN CLAUDE PROJECT]` |
...
- **Token:** [STORED IN CLAUDE PROJECT]
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    ...
    headers := jsonb_build_object(
      ...
      'X-Enrichment-Key', '[STORED IN CLAUDE PROJECT]'
    ),
    body := '{}'::jsonb
  );
  $$
...
### Test Enrichment API
```bat
curl -s -X GET "https://wiotrvoirdgzfacuuiem.supabase.co/functions/v1/enrichment-api?action=audit" -H "X-Enrichment-Key: [STORED IN CLAUDE PROJECT]" -o audit.txt && notepad audit.txt
```
...
```bat
curl -s -H "Authorization: token [STORED IN CLAUDE PROJECT]" -H "Accept: application/vnd.github.v3.raw" "https://api.github.com/repos/rjmjr1962831/list-wise-boost/contents/path/to/file.ts" -o file.ts
```
