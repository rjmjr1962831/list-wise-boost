-- Enable unaccent extension and recreate function
CREATE EXTENSION IF NOT EXISTS unaccent SCHEMA public;

-- Recreate the function now that extension exists
...
BEGIN
  ...
  clean_first := lower(public.unaccent(first_name));
  clean_first := regexp_replace(clean_first, '[^a-z0-9-]', '', 'g');

  -- Clean last name if exists
  IF last_name IS NOT NULL THEN
    clean_last := lower(public.unaccent(last_name));
