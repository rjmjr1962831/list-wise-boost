...
-- Update the trigger function to only sync qualified agents to Pipedrive
...
BEGIN
  ...
  -- Skip if professional is not qualified (4.5+ rating AND 50+ reviews)
  IF NEW.review_stars_rating IS NULL OR NEW.review_stars_rating < 4.5 THEN
    RETURN NEW;
  END IF;
  
  IF NEW.num_total_reviews IS NULL OR NEW.num_total_reviews < 50 THEN
    RETURN NEW;
  END IF;
