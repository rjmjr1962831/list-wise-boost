-- Fix: Add check to prevent re-enqueuing agents that are already in the queue with completed status
-- Also exclude updates to sync-related fields that would cause infinite loops
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    -- Don't queue if already completed recently (within 24h)
    IF NOT EXISTS (
      ...
      DO NOTHING; -- Don't re-enqueue if already pending/processing
    END IF;
    RETURN NEW;
  END IF;

  -- For UPDATE, only queue when AGENT-EDITABLE fields change (not sync metadata)
  IF TG_OP = 'UPDATE' THEN
    -- Skip if this looks like a sync-related update (updated_at changed but no meaningful fields changed)
    -- This prevents the loop where sync-single-professional triggers this again
    ...
    IF (
      ...
      -- Only queue if not already pending/processing AND not recently completed
      IF NOT EXISTS (
        SELECT 1 FROM public.pipedrive_sync_queue 
        WHERE professional_id = NEW.id 
