-- 1) Ensure "I know you're getting bombarded" template has [[BLOCK]] and [your profile]({{profile_url}})
--    so emails get grey callouts and a proper hyperlink instead of raw URL and -- .
-- 2) Backfill magic_link so every row with verification_token has a working dashboard URL.

-- Template: correct body with [[BLOCK]] and [your profile]({{profile_url}})
...
-- Backfill magic_link: canonical dashboard URL = https://www.top10lists.us/dashboard/{verification_token}
UPDATE professionals
SET magic_link = 'https://www.top10lists.us/dashboard/' || verification_token
WHERE verification_token IS NOT NULL
  AND (magic_link IS NULL OR magic_link <> 'https://www.top10lists.us/dashboard/' || verification_token);
