...
serve(async (req) => {
  ...
  try {
    ...
    // Determine badge tier from checkout metadata
    const badgeTier = session.metadata?.badgeTier || null;
    logStep('Badge tier from metadata', { badgeTier });

    // Update professional as brand builder with enhanced tracking + badge tier
    ...
    if (badgeTier) {
      updateFields.badge_tier = badgeTier;
      updateFields.badge_status = 'active';
    }
